...
def get_letter_frequencies(word):
    ...
    """
    to_list = [char for char in word]
    return get_frequencies(to_list)
...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    """
    L1 = [char for char in freq_dict1]
    U = L1 + [char for char in freq_dict2 if char not in freq_dict1]
    sigma = 0
    delta = 0
    for char in U:
        sigma += abs(freq_dict1.get(char,0) + freq_dict2.get(char,0))
        delta += abs(freq_dict1.get(char,0) - freq_dict2.get(char,0))
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    unique_list = [key for key in freq_dict1]
    for key in freq_dict2:
        if key not in unique_list:
            unique_list.append(key)

    for word in unique_list:
        merged[word] = freq_dict1.get(word,0) + freq_dict2.get(word,0)

        if merged[word] > max_freq:
            max_freq = merged[word]

    for key in merged:
        if merged[key] == max_freq:
            most_frequent_word_list.append(key)
    return sorted(most_frequent_word_list)
...
# getting the file for every file path
    for file in file_paths:
        text_file = load_file(file)
        text_list = text_to_list(text_file)

    # get unique character from text list
        unique_list = []
        for char in text_list:
            if char not in unique_list:
                unique_list.append(char)

    # count number of documents with char in it
        for char in unique_list:
            if char not in count_documents:
                count_documents[char] = 1  
            else:
                count_documents[char] += 1
# calculate IDF 
    for key in count_documents:
        idf_dict[key] = math.log10((total_documents/count_documents[key]))
    return idf_dict
...
# loading file to text
    text_file = load_file(tf_file_path)
    list_text = text_to_list(text_file)

# get unique list of text
    unique_list = []
    ...
    for text in unique_list:
        tf_idf_list.append((text,tf_dict[text] * idf_dict[text]))
    tf_idf_list.sort(key=lambda x: (x[1],x[0]))
    return tf_idf_list
